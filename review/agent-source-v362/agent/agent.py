...
_COLOR_CODE_RE = re.compile(r"#?[A-Z]{1,4}\d{2,6}\b")
...
- Cross-border logistics with tracking; size-consult customer service; platform-policy returns; wording kept neutral and compliant.""",
    "ko": """## Market Adaptation Summary
- 대표상: 순백/밝은 원혹 배경; 상세: 정면 -> 마크로 -> 스튜디오 순서.
- 사이즈: 한국 여성복 44-88 참고(본 상품 S부터 55-88 구간); 아시아 헞은 1~2 사이즈 크게 권장; 공급사 실측 cm 볙1표 제공.
- 국제 원스켽 조직이 추적에 담김; 사이즈 상담 고객센터; 플랫폼 반품 규정 준수; 표현은 중성적이고 입직어 없음.""",
    "pt": """## Resumo de Adaptação de Mercado
...
- Logística internacional com rastreio; atendimento para dúvidas de tamanho; devolução conforme política da plataforma; texto neutro e em conformidade.""",
}
...
def a4_market_block(lang, suffix_size_hint, real_measurements=False, kids=False):
    ...
    if kids:
        # v3.6.2 §4：童装口径——成人参考码体系（US women's 0-16 / KR 44-88 / BR P-GG）不得套用
...
def render_buyer_zone(facts, lang, img_ext, flags, img_meta=None):
    ...
    summary, sh = lint_lang(A4_SUMMARY_TMPL[lang], lang)
    bl_hits += sh
    ...
    for i, ln in enumerate(lines):
        ...
        if ln.startswith("## Product Title") and i + 1 < len(lines):
            ...
            lines[i + 1] = t
              ...
              or "색상" in ln):
            ln2 = re.sub(r"#[A-Za-z]{0,10}\d{1,6}\b", "", ln)
            ln2 = re.sub(r"\b[A-Z]{1,4}\d{2,6}\b", "", ln2)
            if ln2 != ln:
                lines[i] = ln2
